...
NeverLose.Scales = {
	...
	Default = UDim2.fromOffset(690 , 480),
...
NeverLose.Font = NeverLose.Fonts.SemiBold;
NeverLose.Theme = {
	BackgroundColor = Color3.fromRGB(8, 8, 13),
	MainColor = Color3.fromRGB(20, 22, 27),
	AccentColor = Color3.fromRGB(78, 127, 252),
	OutlineColor = Color3.fromRGB(45, 48, 58),
	FontColor = Color3.fromRGB(255, 255, 255),
};
NeverLose.ThemeBindings = {};
NeverLose.ThemeCallbacks = {};
NeverLose.IconImages = {};
NeverLose.PendingFlags = {};
NeverLose.IgnoredConfigFlags = {};
NeverLose.IconAssets = {
	["settings"] = "rbxassetid://122669828593160",
	["gear"] = "rbxassetid://122669828593160",
	["palette"] = "rbxassetid://92464809279921",
	["check"] = "rbxassetid://121760666525660",
	["chevron-small-down"] = "rbxassetid://123317177279443",
	["chevron-large-right"] = "rbxassetid://123317177279443",
	["chevron-large-left"] = "rbxassetid://123317177279443",
	["magnifying-glass"] = "rbxassetid://100050851789190",
	["crosshairs"] = "rbxassetid://100050851789190",
	["close"] = "rbxassetid://130510492706892",
	["xmark"] = "rbxassetid://130510492706892",
	["paper-plane"] = "rbxassetid://101636617799068",
	["folder"] = "rbxassetid://103180437044643",
	["floppy-disk"] = "rbxassetid://103180437044643",
	["trash-can"] = "rbxassetid://108839695397679",
	["plus-large"] = "rbxassetid://101500482366184",
	["three-dots-horizontal"] = "rbxassetid://101500482366184",
};
NeverLose.IconColor = NeverLose.Theme.AccentColor;
NeverLose.ScreenGui = GlobalWindow;
NeverLose.Flags = {};
NeverLose.AccentColor = NeverLose.Theme.AccentColor;
NeverLose.MainColor = NeverLose.Theme.BackgroundColor;
...
function NeverLose:AddThread(Thread)
	table.insert(NeverLose.GlobalThreads, Thread);
	return Thread;
end;

function NeverLose:GetDarkerColor(Color)
	return Color:Lerp(Color3.new(0, 0, 0), 0.28);
end;

function NeverLose:MeasureText(Text, TextSize, FontFace, Bounds)
	local Success, Size = pcall(function()
		local Params = Instance.new("GetTextBoundsParams");
		Params.Text = tostring(Text);
		Params.Size = TextSize;
		Params.Font = typeof(FontFace) == "Font" and FontFace or NeverLose.Fonts.Regular;
		local Width = Bounds and Bounds.X or 0;
		Params.Width = Width == math.huge and 0 or Width;
	...
	return TextService:GetTextSize(tostring(Text), TextSize, Enum.Font.GothamMedium, Bounds or Vector2.new(math.huge, math.huge));
end;

function NeverLose:BindTheme(Item, Property, ThemeKey)
	if not Item or not Property or not ThemeKey then
		return Item;
	end;

	table.insert(NeverLose.ThemeBindings, {
		Item = Item,
		Property = Property,
		ThemeKey = ThemeKey,
	});
	...
	pcall(function()
		Item[Property] = NeverLose.Theme[ThemeKey];
	end);

	return Item;
end;

function NeverLose:RegisterThemeCallback(Callback)
	table.insert(NeverLose.ThemeCallbacks, Callback);
	Callback(NeverLose.Theme);
	return Callback;
end;

function NeverLose:UpdateTheme()
	NeverLose.AccentColor = NeverLose.Theme.AccentColor;
	NeverLose.MainColor = NeverLose.Theme.BackgroundColor;
	NeverLose.IconColor = NeverLose.Theme.AccentColor;
	NeverLose.AccentColorDark = NeverLose:GetDarkerColor(NeverLose.Theme.AccentColor);

	for _, Binding in next, NeverLose.ThemeBindings do
		pcall(function()
			Binding.Item[Binding.Property] = NeverLose.Theme[Binding.ThemeKey];
		end);
	end;

	for _, Callback in next, NeverLose.ThemeCallbacks do
		pcall(Callback, NeverLose.Theme);
	end;
end;

function NeverLose:SetTheme(Theme)
	for ThemeKey, Color in next, Theme do
		if NeverLose.Theme[ThemeKey] and typeof(Color) == "Color3" then
			NeverLose.Theme[ThemeKey] = Color;
		end;
	end;

	NeverLose:UpdateTheme();
end;

function NeverLose:GetIconAsset(Icon)
	...
	return NeverLose.IconAssets[string.lower(tostring(Icon))];
end;

function NeverLose:SetIcon(Label, Icon)
	local Asset = NeverLose:GetIconAsset(Icon);
	local Image = NeverLose.IconImages[Label];

	if not Asset then
		if Image then
			Image:Destroy();
			NeverLose.IconImages[Label] = nil;
		end;

		Label:SetAttribute("NeverLoseImageIcon", false);
		NeverLose:SetIconMode(Label, tostring(Icon));
		NeverLose:BindTheme(Label, "TextColor3", "AccentColor");
		return Label;
	end;
	...
	if not Image then
		...
		Image:SetAttribute("NeverLoseThemeIcon", true);
		Image.Parent = Label;
		NeverLose.IconImages[Label] = Image;

		NeverLose:AddSignal(Label:GetPropertyChangedSignal("TextTransparency"):Connect(function()
			Image.ImageTransparency = Label.TextTransparency;
		end));
	end;

	Label:SetAttribute("NeverLoseImageIcon", true);
	Label.Text = "";
	Image.Image = Asset;
	Image.ImageTransparency = Label.TextTransparency;
	NeverLose:BindTheme(Image, "ImageColor3", "AccentColor");
	return Label;
end;

function NeverLose:RegisterFlag(Flag, Object)
	if not Flag then
		return Object;
	end;

	NeverLose.Flags[Flag] = Object;
	if NeverLose.PendingFlags[Flag] ~= nil and Object.SetValue then
		local PendingValue = NeverLose.PendingFlags[Flag];
		NeverLose.PendingFlags[Flag] = nil;
		if Object.Type == "ColorPicker" and typeof(PendingValue) == "string" then
			local Success, Color = pcall(Color3.fromHex, PendingValue:gsub("#", ""));
			if Success then
				PendingValue = Color;
			end;
		end;
		pcall(Object.SetValue, Object, PendingValue);
	end;

	return Object;
end;

local function colorsMatch(Color, Red, Green, Blue)
	return math.abs(Color.R * 255 - Red) < 1
		and math.abs(Color.G * 255 - Green) < 1
		and math.abs(Color.B * 255 - Blue) < 1;
end;

function NeverLose:StyleDescendant(Item)
	if Item:IsA("TextLabel") or Item:IsA("TextButton") or Item:IsA("TextBox") then
		local Family = Item.FontFace and Item.FontFace.Family or "";
		if not string.find(Family, "BuilderIcons", 1, true) then
			if Item.Font == Enum.Font.GothamBold then
				Item.FontFace = NeverLose.Fonts.SemiBold;
			else
				Item.FontFace = NeverLose.Fonts.Regular;
			end;

			if colorsMatch(Item.TextColor3, 255, 255, 255) or colorsMatch(Item.TextColor3, 223, 223, 223) then
				NeverLose:BindTheme(Item, "TextColor3", "FontColor");
			elseif Item.TextColor3 == NeverLose.Theme.AccentColor then
				NeverLose:BindTheme(Item, "TextColor3", "AccentColor");
			end;
		end;
	end;

	if (Item:IsA("ImageLabel") or Item:IsA("ImageButton")) and Item.ImageColor3 == NeverLose.Theme.AccentColor then
		NeverLose:BindTheme(Item, "ImageColor3", "AccentColor");
	end;

	if Item:IsA("UIStroke") and colorsMatch(Item.Color, 45, 48, 58) then
		NeverLose:BindTheme(Item, "Color", "OutlineColor");
	elseif Item:IsA("GuiObject") then
		local Color = Item.BackgroundColor3;
		if Color == NeverLose.Theme.AccentColor then
			NeverLose:BindTheme(Item, "BackgroundColor3", "AccentColor");
		elseif colorsMatch(Color, 8, 8, 13) then
			NeverLose:BindTheme(Item, "BackgroundColor3", "BackgroundColor");
		elseif colorsMatch(Color, 20, 22, 27)
			or colorsMatch(Color, 21, 20, 27)
			or colorsMatch(Color, 25, 27, 33)
			or colorsMatch(Color, 26, 28, 36)
			or colorsMatch(Color, 30, 29, 36)
			or colorsMatch(Color, 39, 40, 49)
			or colorsMatch(Color, 41, 45, 49) then
			NeverLose:BindTheme(Item, "BackgroundColor3", "MainColor");
		end;
	end;
end;

NeverLose:AddSignal(GlobalWindow.DescendantAdded:Connect(function(Item)
	task.defer(function()
		if Item.Parent then
			NeverLose:StyleDescendant(Item);
		end;
	end);
end));
...
NeverLose.CreateInput = LPH_NO_VIRTUALIZE(function(self , Frame , Callback)
	...
	if Callback then
		local bth_signal = NeverLose:AddSignal(Button.MouseButton1Click:Connect(Callback));
...
NeverLose.CreateBlurModule = LPH_NO_VIRTUALIZE(function(self , Frame , Signal)
	...
	local THREAD = NeverLose:AddThread(task.spawn(function()
...
function NeverLose:CreateShadow(parent , RollingEffect)
	...
	Shadow.Render = LPH_NO_VIRTUALIZE(function(self , value)
		...
		if value then
			...
			if RollingEffect then
				RollingEffectThread = NeverLose:AddThread(task.spawn(function()
					local level = 20;
...
function NeverLose:CreateOptionWindow(Frame: Frame , Zindex)
	...
	Window.SetRender = LPH_NO_VIRTUALIZE(function(value)
		...
		if value then
			...
			FollowingThread = NeverLose:AddThread(task.spawn(function()
...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		...
		Toggle.BackgroundColor3 = NeverLose.Theme.MainColor
		...
		ToggleGradient.Rotation = -115
		ToggleGradient.Enabled = false
		ToggleGradient.Parent = Toggle
		NeverLose:RegisterThemeCallback(function(Theme)
			ToggleGradient.Color = ColorSequence.new(Theme.AccentColor, NeverLose:GetDarkerColor(Theme.AccentColor));
		end);
		...
		ToggleLib.SetUI = LPH_NO_VIRTUALIZE(function(value)
			...
			if value then
				NeverLose.PlayAnimate(Toggle,SlowyTween,{
					BackgroundTransparency = 0,
					BackgroundColor3 = NeverLose.Theme.AccentColor
				})
				...
				NeverLose.PlayAnimate(Toggle,SlowyTween,{
					BackgroundTransparency = 0,
					BackgroundColor3 = NeverLose.Theme.MainColor
				})
		...
		NeverLose:RegisterThemeCallback(function()
		...
		if Config.Flag then
			NeverLose:RegisterFlag(Config.Flag, ToggleLib);
		end;
	...
	function handle:AddSlider(Config)
		...
		local SliderLib = { Type = "Slider" };

		SliderLib.GetSize = LPH_NO_VIRTUALIZE(function()
			return (Config.Default - Config.Min) / (Config.Max - Config.Min);
		end);

		local FullNumSize = NeverLose:MeasureText(string.rep("0",(Config.Rounding + #tostring(Config.Max))+1)..tostring(Config.Type), 10, NeverLose.Fonts.Regular, Vector2.new(math.huge, math.huge));
		...
		if Config.Nums then
			...
			local size = NeverLose:MeasureText(string.rep("m",string.len(tostring(ns))), 10, NeverLose.Fonts.Regular, Vector2.new(math.huge, math.huge));
		...
		NeverLose:BindTheme(SlideMoving, "BackgroundColor3", "AccentColor")
		...
		Frame.Size = UDim2.new(0, 16, 0, 12)
		Frame.ZIndex = ZINdex + 15
		Frame.Image = "rbxassetid://117786983271442"
		Frame.ImageColor3 = NeverLose.Theme.FontColor
		NeverLose:BindTheme(Frame, "ImageColor3", "FontColor")
		...
		if Config.Flag then
			NeverLose:RegisterFlag(Config.Flag, SliderLib);
		end;
	...
	function handle:AddOption(GearIcon)
		...
		NeverLose:SetIcon(Icon, Icon.Text)
	...
	function handle:AddColorPicker(Config)
		...
		local ColorPickerLib = { Type = "ColorPicker" };
		...
		if Config.Flag then
			NeverLose:RegisterFlag(Config.Flag, ColorPickerLib);
		end;
	...
	function handle:AddKeybind(Config)
		...
		if Config.Flag then
			NeverLose:RegisterFlag(Config.Flag, KeybindLib);
		end;
	...
	function handle:AddTextInput(Config)
		...
		if Config.Flag then
			NeverLose:RegisterFlag(Config.Flag, TextBoxLib);
		end;
	...
	function handle:AddDropdown(Config)
		...
		NeverLose:SetIcon(DropdownIcon, "chevron-small-down")
		...
		if Config.Flag then
			NeverLose:RegisterFlag(Config.Flag, DropdownLib);
		end;
...
function NeverLose:CreateToolTips(Container: Frame , Name: string , Content: string)
	...
	NeverLose:AddSignal(Container.MouseEnter:Connect(LPH_NO_VIRTUALIZE(function()
		...
		DelayThread = NeverLose:AddThread(task.delay(1,ToolTip.SetRender,true));
	end)));
...
function NeverLose:RegisiterItem(Frame: Frame , Signel)
	...
	function idx:AddButton(Config)
		...
		Icon.TextWrapped = true
		NeverLose:SetIcon(Icon, Config.Icon)
...
function NeverLose:CreateWindow(Config)
	...
	NeverLose:AddThread(task.delay(0.25,function()
	...
	LeftMenuFrame.Size = UDim2.new(0, 225, 1, 0)
	...
	NeverLose:BindTheme(LogoImage, "ImageColor3", "AccentColor")
	...
	LeftScrollingFrame.Size = UDim2.new(1, -24, 1, -75)
	...
	RightMenuFrame.Position = UDim2.new(0, 226, 0, 0)
	RightMenuFrame.Size = UDim2.new(1, -226, 1, 0)
	...
	SettingsButton.Name = NeverLose.RandomString();
	SettingsButton.Parent = RightHeader
	SettingsButton.AnchorPoint = Vector2.new(1, 0.5)
	SettingsButton.AutoButtonColor = false
	SettingsButton.BackgroundColor3 = Color3.fromRGB(20, 22, 27)
	SettingsButton.BackgroundTransparency = 0.25
	SettingsButton.BorderSizePixel = 0
	SettingsButton.Position = UDim2.new(1, -50, 0.5, 0)
	...
	SettingsIcon.Size = UDim2.fromOffset(22, 22)
	SettingsIcon.ZIndex = 13
	SettingsIcon.FontFace = NeverLose.BuiltInBold
	SettingsIcon.Text = "settings"
	SettingsIcon.TextColor3 = NeverLose.Theme.AccentColor
	SettingsIcon.TextSize = 16
	SettingsIcon.TextTransparency = 0.25
	NeverLose:SetIcon(SettingsIcon, "settings")

	Window.SettingsButton = SettingsButton
	Window.SettingsIcon = SettingsIcon
	local SettingsInput = NeverLose:CreateInput(SettingsButton, function()
		if Window.SettingsTab and Window.SelectTab then
			Window:SelectTab(Window.SettingsTab);
		end;
	end);

	NeverLose:AddSignal(SettingsInput.MouseEnter:Connect(function()
		NeverLose.PlayAnimate(SettingsIcon, SlowyTween, { TextTransparency = 0.05 });
	end));
	NeverLose:AddSignal(SettingsInput.MouseLeave:Connect(function()
		local Active = Window.SettingsTab and Window.Tabs[Window.CurrentTab] == Window.SettingsTab;
		NeverLose.PlayAnimate(SettingsIcon, SlowyTween, { TextTransparency = Active and 0 or 0.25 });
	end));
	...
	NeverLose:SetIcon(SearchIcon, "magnifying-glass")
	...
	do
		...
		local Input = NeverLose:CreateInput(SearchIcon , LPH_NO_VIRTUALIZE(function()
			...
			if Window.Searching then
				...
				NeverLose.PlayAnimate(SettingsButton, VSlowTween, {
		...
		NeverLose:AddSignal(SearchBox:GetPropertyChangedSignal('Text'):Connect(LPH_NO_VIRTUALIZE(function()
			...
			last_thread = NeverLose:AddThread(task.delay(max_time,function()
	...
	function Window:AddTabLabel(Name: string)
		...
		TabLabel.Size = UDim2.new(1, -8, 0, 18)
		TabLabel.ZIndex = 8
	...
	function Window:SelectTab(Target)
		local TargetIndex = typeof(Target) == "number" and Target or table.find(Window.Tabs, Target);
		if not TargetIndex or not Window.Tabs[TargetIndex] then
			return false;
		end;

		Window.CurrentTab = TargetIndex;
		for Index, Tab in next, Window.Tabs do
			Tab.SetValue(Index == TargetIndex);
		end;

		local SettingsActive = Window.SettingsTab and Window.Tabs[TargetIndex] == Window.SettingsTab;
		NeverLose.PlayAnimate(SettingsButton, SlowyTween, {
			BackgroundTransparency = SettingsActive and 0 or 0.25,
			BackgroundColor3 = SettingsActive and NeverLose.Theme.AccentColor or NeverLose.Theme.MainColor,
		});
		NeverLose.PlayAnimate(SettingsIcon, SlowyTween, {
			TextTransparency = SettingsActive and 0 or 0.25,
		});
		return true;
	end;
	...
	function Window:AddTab(Config)
		...
		TabButton.Parent = Config.Hidden and nil or LeftScrollingFrame
		...
		SelectedBar.Name = NeverLose.RandomString();
		SelectedBar.Parent = TabButton
		SelectedBar.AnchorPoint = Vector2.new(0, 0.5)
		SelectedBar.BackgroundColor3 = NeverLose.Theme.AccentColor
		...
		SelectedBar.ZIndex = 10
		NeverLose:BindTheme(SelectedBar, "BackgroundColor3", "AccentColor")

		SelectedBarCorner.CornerRadius = UDim.new(1, 0)
		...
		NeverLose:SetIcon(TabIcon, Config.Icon)
		...
		TabContentLabel.Position = UDim2.new(0, 45, 0.5, 0)
		...
		Tab.SetValue = LPH_NO_VIRTUALIZE(function(value)
			...
			if value then
				...
				NeverLose.PlayAnimate(SelectedBar, SlowyTween, {
					BackgroundTransparency = 0,
		...
		if not Config.Hidden and Window.CurrentTab == 0 then
	...
	local function notifySettings(Icon, Message)
		Logging.new(Icon, Message, 3.5);
	end;
	...
	local function normalizeFileName(Name)
		...
		if Name:find("/", 1, true) or Name:find("\\", 1, true) then
			return nil, "name cannot contain path separators";
		end;
		if Name:find("..", 1, true) or Name:find('[<>:"|?*%c]') then
			return nil, "name contains invalid characters";
		end;
		if string.lower(Name) == "autoload.txt" or string.lower(Name) == "themes" then
			return nil, "name is reserved";
		end;
	...
	function SaveManager:SetIgnoreIndexes(Indexes)
		for _, Index in next, Indexes do
			...
			NeverLose.IgnoredConfigFlags[Index] = true;
		end;
	end;

	function SaveManager:GetData(Performance)
		local EncodedFlags = {};
		local Count = 0;

		for Flag, Object in next, NeverLose.Flags do
			if not self.Ignore[Flag] and Object and Object.GetValue then
				local Value = Object:GetValue();
				if typeof(Value) == "Color3" then
					Value = Value:ToHex();
				end;

				table.insert(EncodedFlags, {
					Idx = Flag,
					Type = Object.Type,
					Value = Value,
				});
			...
			Count += 1;
			if Performance and Count % 35 == 1 then
				task.wait();
			end;
		end;

		local Success, Encoded = pcall(function()
			return NeverLose.Base64Encode(Encryption.new(HttpService:JSONEncode(EncodedFlags)));
		end);
		if not Success then
	...
	function SaveManager:DecodeData(Data)
		local Success, Decoded = pcall(function()
			if Data:sub(1, 1) == "{" then
				local Plain = HttpService:JSONDecode(Data);
				return Plain.objects or Plain;
			end;
			return HttpService:JSONDecode(Encryption.reverse(NeverLose.Base64Decode(Data)));
		end);
		if not Success or typeof(Decoded) ~= "table" then
			return nil, "invalid or corrupt config";
		end;
		return Decoded;
	end;

	function SaveManager:LoadData(Data)
		local Decoded, Error = self:DecodeData(Data);
		if not Decoded then
			return false, Error;
		end;
		...
		for _, Entry in next, Decoded do
			if typeof(Entry) ~= "table" then
				return false, "config contains a malformed entry";
			end;
		end;

		for _, Entry in next, Decoded do
			local Flag = Entry.Idx or Entry.idx;
			local Value = Entry.Value;
			if Value == nil then
				Value = Entry.value ~= nil and Entry.value or Entry.text;
			end;

			if Flag and not self.Ignore[Flag] then
				local Object = NeverLose.Flags[Flag];
				if Object and Object.SetValue then
					if Object.Type == "ColorPicker" and typeof(Value) == "string" then
						local ColorSuccess, Color = pcall(Color3.fromHex, Value:gsub("#", ""));
						if ColorSuccess then
							Value = Color;
						end;
					end;
					local Applied = pcall(Object.SetValue, Object, Value);
					if not Applied then
						return false, "failed to apply config flag " .. tostring(Flag);
					end;
				else
					NeverLose.PendingFlags[Flag] = Value;
				end;
			end;
		end;
		return true;
	...
	function SaveManager:RefreshConfigList()
		...
		for _, File in next, Files do
			if isfile(File) then
				...
				if Name and Name ~= "autoload.txt" and not Name:match("%.json$") then
	...
	function SaveManager:Save(Name)
		...
		local CleanName, Error = normalizeFileName(Name);
		if not CleanName then
			return false, Error;
		end;

		local Data, EncodeError = self:GetData();
		if not Data then
			return false, EncodeError;
		end;

		local Success = pcall(writefile, self.Folder .. "/" .. CleanName, Data);
		if not Success then
			return false, "failed to write config";
		end;
		self.SelectedConfig = CleanName;
	...
	function SaveManager:Create(Name)
		...
		if isfile(self.Folder .. "/" .. CleanName) then
			return false, "config already exists";
		end;
		return self:Save(CleanName);
	end;
	...
	function SaveManager:Load(Name)
		...
		local Path = self.Folder .. "/" .. CleanName;
		...
		local Success, LoadError = self:LoadData(Data);
		if not Success then
	...
	function SaveManager:Delete(Name)
		...
		if self.AutoloadName == CleanName then
			self:ClearAutoload();
		end;
		if self.SelectedConfig == CleanName then
	...
	function SaveManager:UpdateAutoloadLabel()
		if self.AutoloadLabel then
			self.AutoloadLabel:SetText("Current autoload config: " .. (self.AutoloadName or "none"));
		end;
	end;
	...
	function SaveManager:SetAutoload(Name)
		...
		if not isfile(self.Folder .. "/" .. CleanName) then
			return false, "config does not exist";
		end;
		local Success = pcall(writefile, self.Folder .. "/autoload.txt", CleanName);
		if not Success then
			return false, "failed to save autoload selection";
		end;
		self.AutoloadName = CleanName;
		self:UpdateAutoloadLabel();
		return true, CleanName;
	end;
	...
	function SaveManager:LoadAutoloadConfig()
		...
		if isfile(AutoloadPath) then
			local Success, SavedName = pcall(readfile, AutoloadPath);
			if Success then
				Name = SavedName;
			end;
		elseif isfile(self.Folder .. "/Default") then
			Name = "Default";
		end;
		...
		self.AutoloadName = CleanName;
		self:UpdateAutoloadLabel();
		return self:Load(CleanName);
	end;

	Window.SaveManager = SaveManager;

	local ThemeManager = {
		Folder = Window.ConfigFolder .. "/themes",
		DefaultTheme = "Prestige Blue",
		CurrentTheme = "Prestige Blue",
		Applying = false,
		ColorControls = {},
		BuiltInThemes = {
			["Prestige Blue"] = { Order = 1, BackgroundColor = "08080d", MainColor = "14161b", AccentColor = "4e7ffc", OutlineColor = "2d303a", FontColor = "ffffff" },
			["Default"] = { Order = 2, BackgroundColor = "141414", MainColor = "1c1c1c", AccentColor = "0055ff", OutlineColor = "323232", FontColor = "ffffff" },
			["BBot"] = { Order = 3, BackgroundColor = "232323", MainColor = "1e1e1e", AccentColor = "7e48a3", OutlineColor = "141414", FontColor = "ffffff" },
			["Fatality"] = { Order = 4, BackgroundColor = "191335", MainColor = "1e1842", AccentColor = "c50754", OutlineColor = "3c355d", FontColor = "ffffff" },
			["Jester"] = { Order = 5, BackgroundColor = "1c1c1c", MainColor = "242424", AccentColor = "db4467", OutlineColor = "373737", FontColor = "ffffff" },
			["Mint"] = { Order = 6, BackgroundColor = "1c1c1c", MainColor = "242424", AccentColor = "3db488", OutlineColor = "373737", FontColor = "ffffff" },
			["Tokyo Night"] = { Order = 7, BackgroundColor = "16161f", MainColor = "191925", AccentColor = "6759b3", OutlineColor = "323232", FontColor = "ffffff" },
			["Ubuntu"] = { Order = 8, BackgroundColor = "323232", MainColor = "3e3e3e", AccentColor = "e2581e", OutlineColor = "191919", FontColor = "ffffff" },
			["Quartz"] = { Order = 9, BackgroundColor = "1d1b26", MainColor = "232330", AccentColor = "426e87", OutlineColor = "27232f", FontColor = "ffffff" },
		},
	};
	...
	function ThemeManager:GetBuiltInList()
		local Names = {};
		for Name in next, self.BuiltInThemes do
			table.insert(Names, Name);
		end;
		table.sort(Names, function(Left, Right)
			return self.BuiltInThemes[Left].Order < self.BuiltInThemes[Right].Order;
		end);
		return Names;
	end;

	function ThemeManager:GetCustomTheme(Name)
		local CleanName = normalizeFileName(Name);
		if not CleanName then
			return nil;
		end;
		local Path = self.Folder .. "/" .. CleanName .. ".json";
		if not isfile(Path) then
			return nil;
		end;
		...
		return Success and Theme or nil;
	end;
	...
	function ThemeManager:DecodeTheme(Theme)
		if typeof(Theme) ~= "table" then
			return nil, "theme data is malformed";
		end;
		local Colors = {};
		for _, ThemeKey in next, { "BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor" } do
			local Hex = Theme[ThemeKey];
			if typeof(Hex) ~= "string" then
				return nil, "theme is missing " .. ThemeKey;
			end;
			local Success, Color = pcall(Color3.fromHex, Hex:gsub("#", ""));
			if not Success then
				return nil, "theme contains an invalid color";
			end;
			Colors[ThemeKey] = Color;
		end;
		return Colors;
	end;
	...
	function ThemeManager:ApplyTheme(Name)
		...
		local Colors, Error = self:DecodeTheme(Theme);
		if not Colors then
			return false, Error;
		end;

		self.Applying = true;
		NeverLose:SetTheme(Colors);
		for ThemeKey, Control in next, self.ColorControls do
			Control:SetValue(Colors[ThemeKey]);
		end;
		if self.BuiltInThemes[Name] and self.PresetControl then
			self.PresetControl:SetValue(Name);
		elseif self.CustomControl then
			self.CustomControl:SetValue(Name);
		end;
		self.Applying = false;
		self.CurrentTheme = Name;
		if self.StatusLabel then
			self.StatusLabel:SetText("Current theme: " .. Name);
		end;
		return true, Name;
	end;
	...
	function ThemeManager:SaveCustomTheme(Name)
		...
		if self.BuiltInThemes[CleanName] then
			return false, "built-in theme names are reserved";
		end;
		local Theme = {};
		for ThemeKey, Color in next, NeverLose.Theme do
			Theme[ThemeKey] = Color:ToHex();
		end;
		local Success = pcall(function()
			writefile(self.Folder .. "/" .. CleanName .. ".json", HttpService:JSONEncode(Theme));
		end);
		if not Success then
	...
	function ThemeManager:SaveDefault(Name)
		...
		local Success = pcall(writefile, self.Folder .. "/default.txt", Name);
		if not Success then
			return false, "failed to save default theme";
		end;
	...
	function ThemeManager:LoadDefault()
		local Name = "Prestige Blue";
		local Path = self.Folder .. "/default.txt";
		if isfile(Path) then
			local Success, SavedName = pcall(readfile, Path);
			if Success and (self.BuiltInThemes[SavedName] or self:GetCustomTheme(SavedName)) then
				Name = SavedName;
			end;
		end;
		self.DefaultTheme = Name;
	...
	NeverLose:RegisterThemeCallback(function(Theme)
		local Active = Window.SettingsTab and Window.Tabs[Window.CurrentTab] == Window.SettingsTab;
		SettingsButton.BackgroundColor3 = Active and Theme.AccentColor or Theme.MainColor;
	end);
	...
	local ThemeColorsSection = SettingsTab:AddSection({ Name = "THEME COLORS", Position = "left" });
	for _, ThemeKey in next, { "BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor" } do
		local Label = ThemeKey:gsub("Color", " Color");
		ThemeManager.ColorControls[ThemeKey] = ThemeColorsSection:AddLabel(Label):AddColorPicker({
			Default = NeverLose.Theme[ThemeKey],
			Flag = "ThemeManager_" .. ThemeKey,
			Callback = function(Color)
				if not ThemeManager.Applying then
					NeverLose:SetTheme({ [ThemeKey] = Color });
					ThemeManager.CurrentTheme = "Custom";
					if ThemeManager.StatusLabel then
						ThemeManager.StatusLabel:SetText("Current theme: Custom");
					end;
				end;
			end,
		});
	...
	local ThemePresetsSection = SettingsTab:AddSection({ Name = "THEME PRESETS", Position = "right" });
	ThemeManager.PresetControl = ThemePresetsSection:AddLabel("Built-in Theme"):AddDropdown({
		Default = "Prestige Blue",
		Values = ThemeManager:GetBuiltInList(),
		Flag = "ThemeManager_ThemeList",
		Size = 140,
		Callback = function(Name)
			if not ThemeManager.Applying and Name then
				local Success, Error = ThemeManager:ApplyTheme(Name);
				if not Success then
					notifySettings("xmark", Error);
				end;
			end;
		end,
	});
	ThemeManager.StatusLabel = ThemePresetsSection:AddLabel("Current theme: Prestige Blue", true);
	ThemePresetsSection:AddButton({
		Name = "Set Current As Default",
		Icon = "check",
		Callback = function()
			local Success, Message = ThemeManager:SaveDefault(ThemeManager.CurrentTheme);
			notifySettings(Success and "check" or "xmark", Success and ("Default theme: " .. Message) or Message);
		end,
	});

	local CustomThemeName = ThemePresetsSection:AddLabel("Custom Theme Name"):AddTextInput({
		Default = "",
		Placeholder = "Theme Name ...",
		Flag = "ThemeManager_CustomThemeName",
		Size = 140,
	});
	ThemeManager.CustomControl = ThemePresetsSection:AddLabel("Custom Theme"):AddDropdown({
		Default = nil,
	...
	ThemePresetsSection:AddButton({
		...
		Icon = "floppy-disk",
		Callback = function()
			local Success, Message = ThemeManager:SaveCustomTheme(CustomThemeName:GetValue());
			if Success then
				ThemeManager.CustomControl:SetValues(ThemeManager:ReloadCustomThemes());
				ThemeManager.CustomControl:SetValue(Message);
			end;
			notifySettings(Success and "check" or "xmark", Success and ("Saved theme " .. Message) or Message);
		end,
	});
	ThemePresetsSection:AddButton({
		...
		Icon = "palette",
		Callback = function()
			local Success, Message = ThemeManager:ApplyTheme(ThemeManager.CustomControl:GetValue());
			notifySettings(Success and "check" or "xmark", Success and ("Loaded theme " .. Message) or Message);
		end,
	});
	ThemePresetsSection:AddButton({
		Name = "Refresh Themes",
		...
		Callback = function()
			ThemeManager.CustomControl:SetValues(ThemeManager:ReloadCustomThemes());
			notifySettings("check", "Refreshed custom themes");
		end,
	});

	local ConfigSection = SettingsTab:AddSection({ Name = "CONFIGURATION", Position = "right" });
	local ConfigNameInput = ConfigSection:AddLabel("Config Name"):AddTextInput({
		Default = "",
		Placeholder = "Config Name ...",
		Flag = "SaveManager_ConfigName",
		Size = 140,
	});
	local ConfigList = ConfigSection:AddLabel("Config List"):AddDropdown({
		Default = nil,
		Values = SaveManager:RefreshConfigList(),
		Flag = "SaveManager_ConfigList",
		Size = 140,
		Callback = function(Name)
			SaveManager.SelectedConfig = Name;
		end,
	});
	SaveManager.AutoloadLabel = ConfigSection:AddLabel("Current autoload config: none", true);

	local function refreshConfigs(Selected)
		ConfigList:SetValues(SaveManager:RefreshConfigList());
		if Selected then
			ConfigList:SetValue(Selected);
		end;
	end;
	...
	ConfigSection:AddButton({
		...
		Icon = "plus-large",
		Callback = function()
			local Success, Message = SaveManager:Create(ConfigNameInput:GetValue());
			if Success then refreshConfigs(Message); end;
			notifySettings(Success and "check" or "xmark", Success and ("Created " .. Message) or Message);
		end,
	});
	ConfigSection:AddButton({
		Name = "Load Config",
		Icon = "folder",
		Callback = function()
			local Success, Message = SaveManager:Load(ConfigList:GetValue());
			notifySettings(Success and "check" or "xmark", Success and ("Loaded " .. Message) or Message);
		end,
	});
	ConfigSection:AddButton({
		...
		Icon = "floppy-disk",
		Callback = function()
			local Success, Message = SaveManager:Save(ConfigList:GetValue());
			notifySettings(Success and "check" or "xmark", Success and ("Saved " .. Message) or Message);
		end,
	});
	ConfigSection:AddButton({
		Name = "Delete Config",
		Icon = "trash-can",
		Callback = function()
			local Success, Message = SaveManager:Delete(ConfigList:GetValue());
			if Success then refreshConfigs(); end;
			notifySettings(Success and "check" or "xmark", Success and ("Deleted " .. Message) or Message);
		end,
	});
	ConfigSection:AddButton({
		...
		Callback = function()
			refreshConfigs();
			notifySettings("check", "Refreshed configs");
		end,
	});
	ConfigSection:AddButton({
		...
		Icon = "check",
		Callback = function()
			local Success, Message = SaveManager:SetAutoload(ConfigList:GetValue());
			notifySettings(Success and "check" or "xmark", Success and ("Autoload: " .. Message) or Message);
		end,
	});
	...
	NeverLose:AddThread(task.delay(1, function()
		local Success, Error = SaveManager:LoadAutoloadConfig();
		if not Success then
			notifySettings("xmark", "Autoload failed: " .. tostring(Error));
		end;
	end));
	...
	function Window:Watermark()
		...
		function Watermark_lb:AddBlock(IconStr , Name)
			...
			Icon.TextTransparency = 0.250
			Icon.TextWrapped = true
			NeverLose:SetIcon(Icon, IconStr)

			InnerBlock.Update = LPH_NO_VIRTUALIZE(function(value)
		local size = NeverLose:MeasureText(Content.Text, Content.TextSize, Content.FontFace, Vector2.new(math.huge, math.huge))
...
function NeverLose:CreateNotification()
	...
	function Notifier.new(Config)
		...
		NeverLose:AddThread(task.delay(Config.Duration or 5 , LPH_NO_VIRTUALIZE(function()
...
function NeverLose:CreateLogger()
	...
	function Logging.new(IconStr: string , Message: string , Duration: number)
		...
		NeverLose:AddThread(task.delay(0.15,LPH_NO_VIRTUALIZE(function()
			Shadow:Render(true);
...
function NeverLose:Unload()
	...
	table.clear(NeverLose.ThemeCallbacks);
	table.clear(NeverLose.ThemeBindings);
	table.clear(NeverLose.PendingFlags);
